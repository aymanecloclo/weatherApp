...
const SearchBox = ({ onChange }) => {
  ...
  const fetchCities = async (inputValue) => {
    if (inputValue.length > 2) { // Attendre que l'utilisateur tape au moins 3 lettres
      try {
        ...
        // Utilisation d'un Set pour s'assurer qu'il n'y a pas de doublons
        const uniqueCities = [...new Set(data.list.map(cityObj => cityObj.name))];
        setFilteredCities(uniqueCities); // Récupérer les noms des villes uniques
      } catch (error) {
  ...
  // Fonction pour récupérer les villes en fonction de la saisie
  }

  return (
    <div className='flex justify-center'>
      <div className="absolute w-full lg:w-9/12 lg:top-[5%] bg-white min-screen z-50 flex flex-col rounded-lg lg:min-h-[80%] border shadow-md">
        ...
        <ul className='flex flex-col mt-5'>
          ...
          {filteredCities.length > 0 ? (
            filteredCities.map((cityName, index) => (
              <li key={index} onClick={catchValue} className='py-2 w-11/12 border-b-2 cursor-pointer text-sm px-3'>{cityName}</li>
            ))
          ) : (
